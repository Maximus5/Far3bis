...
static FP_DECL_DIALOG( ColorDialog )
  ...
  {  FarDialogItemData id;
    ...
    switch( Msg ) {
      case        DN_BTNCLICK: if ( Param1 >= cdlgFORE && Param1 < cdlgFORE+16 )
                               ...
                               if ( Param1 >= cdlgBK && Param1 < cdlgBK+16 )
...
int DECLSPEC FP_GetColorDialog( int color,PFLngColorDialog p,CONSTSTR Help )
  {  FarDialogItem DialogItems[ FP_DIALOG_SIZE(ColorDialog) ];
     ...
     n = FP_Info->DialogEx( FP_Info->ModuleNumber,-1,-1,39,15,Help,
                            DialogItems,FP_DIALOG_SIZE(ColorDialog),0,0,
                            CDLG_WndProc,0 );
     if ( n == cdlgOK )
