...
int FP_Multiline::AddSelection( void )
  {
     ...
     Selections = (PFP_Hilight)_Realloc( Selections,sizeof(FP_Hilight)*(SelCount+1) );
     Assert( Selections );
...
PFP_Hilight FP_Multiline::GetSelection( int num )
  {
...
bool FP_Multiline::GoTo( int y,int x,bool ForceUpdate )
  {  int oldy = CurrentLine + top,
    ...
    if ( y < top )                 nTop = y; else
    if ( y-nTop >= ItemsCount )    nTop = Max( 0,y-ItemsCount+1 );
...
enum SPos {
  ...
  SNone
     ...
     if ( sel->X()  == x && sel->Y()  == y ) pos = SStart; else
     if ( sel->X1() == x && sel->Y1() == y ) pos = SEnd; else
       pos = SNone;
...
#define DRET( v ) { rc = v; return TRUE; }
    switch( Msg ) {
      case DN_INITDIALOG: Editor = (PFP_Editor)Param2;
