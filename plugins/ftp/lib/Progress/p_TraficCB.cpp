...
extern CONSTSTR StdCommands[];

CONSTSTR StdDialogLines[] = {
  "%*\xFA-61SrcPathname%",
  "%*\xFA-61DestPathname%",
...
void TrafficInformation::FormatLine( int num,CONSTSTR line,time_t tm )
  {  char     str[50];
     int      n,cn;
     InfoItem it;
    ...
    for ( n = 0; *line && n < MAX_TRAF_WIDTH; line++ ) {
      ...
      if ( *line == '*' ) {
        ...
        it.Fill  = ' ';
      //Align
      if ( *line == '-' ) it.Align = tLeft; else
      if ( *line == '+' ) it.Align = tRight; else
      if ( *line == '.' ) it.Align = tCenter; else
      if ( *line == '>' ) {
      //Right line
        line++;
        it.Fill = *line++;
...
void TrafficInformation::DrawInfos( time_t tm )
  {  char str[ MAX_TRAF_WIDTH+1 ];
    ...
    SNprintf( key,sizeof(key),"CopyDialog\\%s",FTP_Info->GetMsg(MLanguage) );
...
void TrafficInformation::Init( HANDLE h,int tMsg,int OpMode,PFP_SizeItemList il )
  {
...
void TrafficInformation::InitFile( __int64 sz,CONSTSTR SrcName,CONSTSTR DestName )
  {
    if ( FullFileSize != -1 ) {
      TotalComplete++;
...
void TrafficInformation::Resume( CONSTSTR nm )
  {  WIN32_FIND_DATA ffi;
...
//Show message
    ...
    FTP_Info->FMessage( FMSG_LEFTALIGN, NULL,
                        (CONSTSTR *)MsgItems, LineCount+1, 0 );
