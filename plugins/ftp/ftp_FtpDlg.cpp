...
//---------------------------------------------------------------------------------
BOOL DECLSPEC AskSaveList( PSaveListInfo sli )
  {
...
FP_END_DIALOG
     ...
     if ( FDialog( 60,16,"FTPSaveList",DialogItems,FP_DIALOG_SIZE(InitItems) ) != 16 )
       return FALSE;

     StrCpy( sli->path, DialogItems[ 2].Data, FAR_MAX_PATHSIZE );

     if ( DialogItems[ 4].Selected ) sli->ListType = sltUrlList; else
     if ( DialogItems[ 5].Selected ) sli->ListType = sltTree;    else
     if ( DialogItems[ 6].Selected ) sli->ListType = sltGroup;
...
//---------------------------------------------------------------------------------
BOOL FTP::CopyAskDialog( BOOL Move, BOOL Download, PFTPCopyInfo ci )
  {
static FP_DECL_DIALOG( InitItems )
...
//Dialog
     if ( FDialog( 76,19,"FTPCmd",di,FP_DIALOG_SIZE(InitItems) ) != MID_DEFB ) {
...
//---------------------------------------------------------------------------------
void QueryExHostOptions( PFTP ftp,PFTPHost p )
  {
static FP_DECL_DIALOG( InitItems )
...
#define dCONNECT 26
  ...
  do{
    rc = FDialog( 76,19,"FTPConnect",di,FP_DIALOG_SIZE(InitItems) );
    if ( rc == -1 )
      return FALSE;
...
FP_END_DIALOG
   ...
   //Exec
       int AskCode = FDialog( 76,8,"FTPCmd",di,FP_DIALOG_SIZE(InitItems) );
...
FP_END_DIALOG
  ...
  AskCode = FDialog( 39,8,"FTPCmd",di,FP_DIALOG_SIZE(InitItems) );
  if ( AskCode != 12 )
    return;
...
//---------------------------------------------------------------------------------
BOOL DECLSPEC GetLoginData( char *User, char *Password, BOOL forceAsk )
{
static FP_DECL_DIALOG( InitItems )
