...
StringData<T> *eus()
{
	//дл€ оптимизации создани€ пустых String
...
public:
	...
	StringBase(const wchar_t* lpData, int nCodePage)
	{
		m_pData = NULL;
		SetData(lpData, nCodePage);
	}
	...
	StringBase& SetData(const wchar_t* lpData, size_t nLength, int nCodePage = CP_OEMCP)
	{
		if ( !lpData || !nLength )
		{
			FreeData();
		...
		{
			size_t nSize = Traits::GetUnicodeDataLength(lpData, nLength, nCodePage);

			if ( m_pData && m_pData->Unique() && (nSize < m_pData->GetSize()) )
			{
				Traits::CopyUnicodeData(m_pData->GetData(), nSize, lpData, nLength, nCodePage);
	...
	StringBase& SetData(const char* lpData, size_t nLength, int nCodePage = CP_OEMCP)
	{
		...
		{
			size_t nSize = Traits::GetAnsiDataLength(lpData, nLength, nCodePage);

			if ( m_pData && m_pData->Unique() && (nSize < m_pData->GetSize()) )
			{
				Traits::CopyAnsiData(m_pData->GetData(), nSize, lpData, nLength, nCodePage);
				m_pData->SetLength(nLength);
			}
			else
			{
